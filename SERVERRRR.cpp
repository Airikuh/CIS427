...
void* serverCommands(void* userData) {
    ...
    while (1)
    {
        ...
        // Check that the client used the command properly
            ...
            // Check that selected user exists in Users table 
                ...
                // Get the usd balance of the user
                    ...
                    // Check that record already exists in Pokemon_cards
                        ...
                        // A record exists, so update the record
                                sql = "UPDATE Pokemon_cards SET card_price= card_price +" + infoArr[1] + " WHERE Pokemon_cards.card_name='" + infoArr[0] + "' AND Pokemon_cards.ID='" + id + "' AND Pokemon_cards.owner_id='" + id + "';";
                        ...
                        // A record does not exist, so add a record
                                sql = "INSERT INTO Pokemon_cards(card_name, card_price, owner_id) VALUES ('" + infoArr[0] + "', '" + infoArr[1] + "', '" + id + "');";
                                rc = sqlite3_exec(db, sql.c_str(), NULL, NULL, &zErrMsg);
                    ...
                    // Get the new card_price
                            sql = "SELECT card_price FROM Pokemon_cards WHERE Pokemon_cards.card_name='" + infoArr[0] + "' AND Pokemon_cards.owner_id='" + id + "';";
                            rc = sqlite3_exec(db, sql.c_str(), callback, ptr, &zErrMsg);
                            ...
                            string card_price = result;

                    // The command completed successfully, return 200 OK, the new usd_balance and new card_price
                            string tempStr = "200 OK\n   BOUGHT: New balance: " + card_price + " " + infoArr[0] + ". USD balance $" + usd_balance;
        ...
        // Check that the client used the command properly
                if (!extractInfo(Buff, infoArr, command)) {
                    ...
                    send(clientID, "400 Invalid Command: Missing information\n EX. Command: SELL card_name Card_price quantity userID", sizeof(Buff), 0);
                }
                else {
            // Check that the selected user exists in Users table 
                ...
                // Check that the user owns the selected card
                        sql = "SELECT IIF(EXISTS(SELECT 1 FROM Pokemon_cards WHERE Pokemon_cards.card_name='" + infoArr[0] + "' AND Pokemon_cards.owner_id='" + id + "'), 'RECORD_PRESENT', 'RECORD_NOT_PRESENT') result;";
                        rc = sqlite3_exec(db, sql.c_str(), callback, ptr, &zErrMsg);
                    ...
                    // Check that the user has enough of the selected card to sell
                            ...
                            //int numCardsToSell = stod(infoArr[1]);

//trying 2 instead
                            int numCardsToSell = stod(infoArr[2]);
                    ...
                    // Get the number of cards the user owns of the selected card
                            ...
                            double card_price = stod(result);
                    // Not enough cards in balance to sell
                            if (card_price < numCardsToSell) {
                        ...
                        // Remove the sold cards from Pokemon_cards
                                sql = "UPDATE Pokemon_cards SET quantity= quantity -" + to_string(numCardsToSell) + " WHERE Pokemon_cards.card_name='" + infoArr[0] + "' AND Pokemon_cards.owner_id='" + id + "';";
                                rc = sqlite3_exec(db, sql.c_str(), NULL, NULL, &zErrMsg);
                        ...
                        // Get new SOLD
                               ...
                               sql = "SELECT quantity FROM Pokemon_cards WHERE Pokemon_cards.card_name='" + infoArr[0] + "' AND Pokemon_cards.owner_id='" + id + "';";
cout << "Infor array 0: " << infoArr[0] << endl;
cout <<  "Infor array 1: " <<  infoArr[1] << endl;
cout <<  "Infor array 2: " <<  infoArr[2] << endl;

                                rc = sqlite3_exec(db, sql.c_str(), callback, ptr, &zErrMsg);
                                string quantity = result;

                        // Sell command completed successfully, changed here, numbercards sold
                                //card_price infoArry[0]

                                //string tempStr = "200 OK\n   SOLD: New balance: " + quantity + " " + infoArr[2] + ". USD $" + usd_balance;
    	   ...
    	   //cout << "Received: LOGOUT" << endl;
            ...
            // get full user name
                ...
                string count = result;
                result = "";
                sql = "SELECT card_name, card_price FROM (SELECT * FROM Pokemon_cards WHERE owner_id = " + id + ") WHERE card_name LIKE '%" + searchTerm + "%'";
                rc = sqlite3_exec(db, sql.c_str(), callback, ptr, &zErrMsg);
                ...
                else
                {
                    ...
                    if (command == "LOGIN") {
                        ...
                        if (result == "USER_PRESENT") {
                            ...
                            pthread_create(&(list.at(list.size() - 1).myThread), NULL, serverCommands, temp);
