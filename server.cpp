...
// Adds Charizard to Pokemon Database
...
// Adds Bulbasaur to Pokemon Database
...
// Adds Squirtle to Pokemon Database
...
// Checks if the client used the command properly
                if (!extractInfo(Buff, clientData, command)) {
                    send(clientID, "400 Invalid Command: Missing information\n EX. Command: BUY card_name card_type rarity quantity price userID", sizeof(Buff), 0);
...
//  Add New Record Or Update Record To Pokemon Cards Table As Well As Checks If Record Already Exists In Pokemon_Cards
 ...
 //  A record exists, so update the record
                                sql = "UPDATE Pokemon_cards SET card_quantity= card_quantity +" + clientData[3] + " WHERE Pokemon_cards.card_name='" + clientData[0] + "' AND Pokemon_cards.owner_id='" + id + "';";
                                rc = sqlite3_exec(db, sql.c_str(), NULL, NULL, &zErrMsg);
                                cout << "Added " << clientData[0] << " Card to " << clientData[0] << " for " << id << endl;
// Check if SQL executed correctly
                                if (rc != SQLITE_OK) {
                                    fprintf(stderr, "SQL error: %s\n", zErrMsg);
                                    sqlite3_free(zErrMsg);
...
//Confirm correct quantity in command input
            if (i > 0) {
